...
function getAllProducts($fromLt, $per_page , $sortfield, $orderby){
     ...
     $Q = $this->db->query('SELECT p.*,m.name as maker_name,c.title as model_name FROM products p, carmodel c 
		LEFT JOIN maker m ON m.id = c.maker WHERE p.model = c.id order by '.$sortfield.' '.$orderby.' limit '.$fromLt.','.$per_page.' ');
...
if(!empty($searchby) && $searchby == 'part_number') {$product_like = " AND  p.part_number like '%".$searchKeyword."%' "; } // product part number
		...
		$Q = $this->db->query('SELECT p.*,m.name as maker_name,c.title as model_name FROM products p, carmodel c 
		LEFT JOIN maker m ON m.id = c.maker WHERE p.model = c.id '.$model_search.' '.$group_search.' '.$make_search.' '.$product_like.' '.$year_search.' '.$price_search.''.$status_search.' order by '.$sortfield.' '.$orderby.' '.$limit.'');
