...
{  
	...
	if(confirm("Do you want to delete Records??"))
	{
		...
		else
		{
			...
			$.ajax({
				  type: "POST",
				  url: "<?=base_url()?>admin/categories/deleteMultiple",
				  data: { id: str },
...
<p>Create, edit, delete and manage Groups on your online store.</p>
<a href="<?=base_url()?>admin/categories/create" title="Create New Group" class="createCategory">Create New Group</a>
...
if (count($categories)){
	...
	foreach ($categories as $key => $list){
		...
		echo "<a href='".base_url()."admin/categories/edit/".$list['id']."' class='icons' title='Edit'><img src='".base_url()."images/icons/edit.png' /></a>";
		//echo " | ";
		//echo anchor('admin/categories/delete/'.$list['id'],'delete');
		echo "<a href='".base_url()."admin/categories/delete/".$list['id']."' class='icons' title='Delete' onClick=\"return deletechecked('Are you sure you want to delete this group ?');\"><img src='".base_url()."images/icons/delete.png' /></a>";
		echo "</td>\n";
		echo "</tr>\n";
	}
	echo "</tbody>\n</table>";
...
var reqUrl = "<?=base_url()?>admin/categories/udpateCategoryStatus/"+productId+"/"+status_val1
$.ajax({
  url: reqUrl,
