...
class Payments extends Model
{
   ...
   function Payments(){
	...
	function Do_direct_payment_demo()
	{
		...
		$CCDetails = array(
							'creditcardtype' => $_POST['CardType'], 					// Required. Type of credit card.  Visa, MasterCard, Discover, Amex, Maestro, Solo.  If Maestro or Solo, the currency code must be GBP.  In addition, either start date or issue number must be specified.
							'acct' =>  $_POST['cardNo'], 								// Required.  Credit card number.  No spaces or punctuation.  
							'expdate' =>  $_POST['month']."20".$_POST['year'], 							// Required.  Credit card expiration date.  Format is MMYYYY
							'cvv2' =>  $_POST['ccv'], 								// Requirements determined by your PayPal account settings.  Security digits for credit card.
							'startdate' => '', 							// Month and year that Maestro or Solo card was issued.  MMYYYY
							'issuenumber' => ''							// Issue number of Maestro or Solo card.  Two numeric digits max.
						);
		...
		$PayerName = array(
							...
							'firstname' => $_SESSION['customer_first_name'], 							// Payer's first name.  25 char max.
							'middlename' => '', 						// Payer's middle name.  25 char max.
							'lastname' => $_SESSION['customer_last_name'], 							// Payer's last name.  25 char max.
							'suffix' => ''								// Payer's suffix.  12 char max.
						);
		...
		$BillingAddress = array(
								...
								'zip' =>  $_POST['post_code'], 							// Required.  Postal code of payer.
								'phonenum' =>  $_POST['phone_number']						// Phone Number of payer.  20 char max.
							);
							
		$ShippingAddress = array(
								'shiptoname' => $_POST['addressS'], 					// Required if shipping is included.  Person's name associated with this address.  32 char max.
								'shiptostreet' => $_POST['address'], 					// Required if shipping is included.  First street address.  100 char max.
								'shiptostreet2' => '', 					// Second street address.  100 char max.
								'shiptocity' => $_POST['cityS'], 					// Required if shipping is included.  Name of city.  40 char max.
								'shiptostate' => $_POST['stateS'], 					// Required if shipping is included.  Name of state or province.  40 char max.
								'shiptozip' => $_POST['post_codeS'], 						// Required if shipping is included.  Postal code of shipping address.  20 char max.
								'shiptocountry' => 'US', 					// Required if shipping is included.  Country code of shipping address.  2 char max.
								'shiptophonenum' => $_POST['phone_numberS']					// Phone number for shipping address.  20 char max.
								);
							$tot = 0;
							$tot = round($_SESSION['subtotalprice']+$_SESSION['TAX'],2);
		$PaymentDetails = array(
								'amt' => $tot , 	// Required.  Total amount of order, including shipping, handling, and tax.  
								'currencycode' => 'USD', 					// Required.  Three-letter currency code.  Default is USD.
								'itemamt' => round($_SESSION['subtotalprice'],2), 						// Required if you include itemized cart details. (L_AMTn, etc.)  Subtotal of items not including S&H, or tax.
								'shippingamt' => "0", 					// Total shipping costs for the order.  If you specify shippingamt, you must also specify itemamt.
								'shipdiscamt' => '', 					// Shipping discount for the order, specified as a negative number.  
								'handlingamt' => '', 					// Total handling costs for the order.  If you specify handlingamt, you must also specify itemamt.
								'taxamt' => round($_SESSION['TAX'],2), 						// Required if you specify itemized cart tax details. Sum of tax for all items on the order.  Total sales tax. 
								'desc' => 'Power Steering Kit Specialist purchase', 							// Description of the order the customer is purchasing.  127 char max.
								'custom' => '', 						// Free-form field for your own use.  256 char max.
								'invnum' => 'Twinkle-'.$_SESSION['orderId'], 						// Your own invoice or tracking number
								'notifyurl' => ''						// URL for receiving Instant Payment Notifications.  This overrides what your profile is set to use.
							);	
		...
		$Item	 = array(
							'l_name' => $row['name'], 						// Item Name.  127 char max.
							'l_desc' => $row['name'], 						// Item description.  127 char max.
							'l_amt' => round($row['price'],2), 							// Cost of individual item.
							'l_number' => $row['productid'], 						// Item Number.  127 char max.
							'l_qty' => $row['count'], 							// Item quantity.  Must be any positive integer.  
							'l_taxamt' => '', 						// Item's sales tax amount.
							'l_ebayitemnumber' => '', 				// eBay auction number of item.
							'l_ebayitemauctiontxnid' => '', 		// eBay transaction ID of purchased item.
							'l_ebayitemorderid' => '' 				// eBay order ID for the item.
		...
		if(!$this->paypal_pro->APICallSuccessful($PayPalResult['ACK']))
		{
			return $data = array('Errors'=>$PayPalResult['ERRORS']);
	...
	function Set_express_checkout()
	{
		...
		$Payment = array(
						...
						'itemamt' => round($_SESSION['subtotalprice'],2), 						// Required if you specify itemized L_AMT fields. Sum of cost of all items in this order.  
						...
						'taxamt' => round($_SESSION['TAX'],2), 						// Required if you specify itemized L_TAXAMT fields.  Sum of all tax items in this order. 
