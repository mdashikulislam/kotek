...
{  
	...
	if(confirm("Do you want to delete Records??"))
	{
		...
		else
		{
			...
			$.ajax({
				  type: "POST",
				  url: "<?=base_url()?>admin/banner/deleteMultiple",
				  data: { id: str },
...
<p>Create, edit, delete and manage Banners on your online store.</p>
<p>Banner reflects on Home page.</p>
<a href="<?=base_url()?>admin/banner/create" title="Create New Banner" class="createBanner">Create New Banner</a>
...
echo "<td>".form_checkbox('cat_id[]',$list['id'],FALSE,'class="case"')."</td>\n";
		...
		echo "<td align='center'><input type='text' name='plc_order' value='".$list['place_order']."'style='width: 20px;' onchange='updateOrder(".$list['id'].",this.value);'  /></td>\n";

		echo "<td align='center'>";

		//echo anchor('admin/banner/edit/'.$list['id'],'edit');

		echo "<a href='".base_url()."admin/banner/edit/".$list['id']."' class='icons' title='Edit'><img src='".base_url()."images/icons/edit.png' /></a>";
		...
		echo "<a href='".base_url()."admin/banner/delete/".$list['id']."' class='icons' title='Delete' onClick=\"return deletechecked('Are you sure you want to delete this banner ?');\"><img src='".base_url()."images/icons/delete.png' /></a>";
