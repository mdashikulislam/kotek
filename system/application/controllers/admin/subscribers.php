...
class Subscribers extends Controller {
  ...
  function deleteMultiple(){
	...
	{
	   $this->MSubscribers->removeSubscriber($row);
	}
	$this->session->set_flashdata('message','Subscribers deleted sucessfully');
...
$msg = "Hi, <br/> Admin unsubscribe you to Power Steering Kit Specialist.<br/><br/> <br/>Thanks,<br/>Administrator, Power Steering Kit Specialist
 ...
 /*********************************  section for user request ****************************************/
	...
	$this->load->view('dashboard');  
  	...
  	function deleteMultipleRequest(){
	$id_str = $_POST['id'];
	$deleteArr = explode(',',$id_str);
	if(!empty($deleteArr)){
        foreach($deleteArr as $row)
	{
	$this->MSubscribers->removeRequest($row);
	}
	$this->session->set_flashdata('message','Requests deleted sucessfully');
