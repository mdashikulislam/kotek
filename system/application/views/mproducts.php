...
function getProductOrder($id){
 ...
 function getAllProducts($fromLt, $per_page){
     $data = array();
	 
     $Q = $this->db->query('SELECT p.*,m.name as maker_name,c.title as model_name FROM products p, carmodel c 
		LEFT JOIN maker m ON m.id = c.maker WHERE p.model = c.id order by p.id limit '.$fromLt.','.$per_page.' ');
...
if(!empty($searchby) && $searchby == 'part_number') {$product_like = " AND  p.part_number like '%".$searchKeyword."%' "; } // product part number
		...
		$Q = $this->db->query('SELECT p.*,m.name as maker_name,c.title as model_name FROM products p, carmodel c 
		LEFT JOIN maker m ON m.id = c.maker WHERE p.model = c.id '.$model_search.' '.$group_search.' '.$make_search.' '.$product_like.' '.$year_search.' '.$price_search.''.$status_search.' '.$limit.'');
		
		return $Q->num_rows();	
	 
  }
 function getAllProductsSearch($searchKeyword,$searchby,$from,$per_page)
 { 
		$model_search =""; $make_search = ""; $product_like ="";  $year_search =""; $group_search ="";  $price_search =""; $status_search ="";
		if(!empty($searchby) && $searchby == 'model') {$model_search = " AND  c.title like '%".$searchKeyword."%' "; } // car model
		if(!empty($searchby) && $searchby == 'maker') {$model_search = " AND  m.name like '%".$searchKeyword."%' "; } // maker
 ...
 function addProduct(){
	...
	/* if ($_FILES){
		...
		if (strlen($_FILES['image']['name'])){
			...
			if ($image['file_name']){
				$data['image'] = "images/product/".$image['file_name'];
		
			}
		}
				
		if (strlen($_FILES['thumbnail']['name'])){
						$config['upload_path'] = './images/thumb/';
						$config['allowed_types'] = 'gif|jpg|png';
						$config['max_size'] = '200';
						$config['remove_spaces'] = true;
						$config['overwrite'] = false;
			...
			if ($thumb['file_name']){
				$data['thumbnail'] = "images/thumb/".$thumb['file_name'];
...
function getSettings()
{
	...
	$model_search =""; $make_search = ""; $product_like ="";  $year_search =""; $partno_search ="";
