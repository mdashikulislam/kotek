...
class Dimensions extends Controller {
  ...
  function delete($id){
	...
	$this->MDimensions->deleteDimensions($row);
	}
	$this->session->set_flashdata('message','Dimensions deleted sucessfully');
  ...
  function export(){
	...
	function addDimentions($counter = 0)
	{
		...
		echo '<div class="singleDimention"><div class="dimenSelect"><select name="dimentionsList[]" >';
		...
		echo '</select></div>';
		echo '<div class="dimenValue"><input type="text" class="textfield" id="dimensions" value="" name="dimensions[]"> </div><a href="#" class="button_remove">Delete</a>';
		echo '</div>';
